...
1. **Hermes → InboxScout as an MCP server (recommended).** Turn on *Setup → Preferences → Who can help → Let other AI tools on this computer use InboxScout* in
...
3. **InboxScout → Hermes.**
   - *Every brief, pushed:* set a webhook URL under *Preferences → Who can help → Also send each brief to another program* and InboxScout POSTs each new brief
...
sees it. The person chooses how far the Assistant goes in *Setup → Web chores*: **Full** (default), **Sign in for me**,
or **Careful**. Verification codes always come back to the person.
