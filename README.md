...
| 🩹 **Fixes itself** | InboxScout checks its own health on every run and repairs what it safely can — reconnects an account with a saved sign-in, falls back to the built-in engine when the AI helper fails, moves the reports folder if it vanished, picks a free port for the bridge, re-syncs a broken account. You see one calm line ("Fixed on its own: …"), and when something really needs you, Settings → Health says so in plain words with one **Fix it for me** button. [docs/SELF-HEALING.md](docs/SELF-HEALING.md) |
...
| 🧭 **Knows who you are** | 56 profiles — nurse, contractor, teacher, landlord, retiree, developer, pastor, caregiver, restaurant owner and 47 more — each tuning what counts as work, what's urgent, and what your brief tracks. Pick one, or let InboxScout figure it out from your mail and dial it back any time. [docs/PROFILES.md](docs/PROFILES.md) |
| 👥 **Knows your people & your week** | Learns your circle across every inbox — family, friends, clients, vendors — treats inner-circle mail as important on its own, and quietly tells you when someone has gone quiet. One schedule for the week from every inbox with overlaps, overdue items, and the regulars it notices. Promises you made in your own mail, with follow-ups. Per-inbox counts. Nothing to set up. [docs/INSIGHTS.md](docs/INSIGHTS.md) |
...
| 👵 **Simple for a grandparent, sharp for a CEO** | The layout evolves with you: **Simple** (one big button, three cards, spoken help), **Standard**, or **Pro** (decisions first, every inbox, delegation, keyboard). Picked from how you use it, changed at most once a week with a way back, or locked by you. Every setting explains what it does and why. [docs/ADAPTIVE.md](docs/ADAPTIVE.md) |
...
| 🤖 **Assistant** | An AI-operated browser that does web chores for you in its own window: creates the Gmail/Yahoo/iCloud app password and connects the account, registers the Google/Microsoft sign-in apps, adds family members — or any task you describe. Save a sign-in and it logs in for you (the AI never sees the password). Full autonomy by default; dial it back to Sign in for me or Careful any time. Every step is visible with a Stop button. [docs/ASSISTANT.md](docs/ASSISTANT.md) |
| 🖥 **Uses your whole computer** | The Assistant (and a connected agent) can open apps and files, click and type on your desktop, run commands, and read or write files under your home folder — on by default. The first time it does each kind of thing, a popup asks (Allow once / Always allow / Don't allow); dangerous commands ask unless the full-autonomy choice is on (it is on by default; turn it off in Settings → Who can help); secret folders are off-limits. One switch turns it off. [docs/SYSTEM-CONTROL.md](docs/SYSTEM-CONTROL.md) |
